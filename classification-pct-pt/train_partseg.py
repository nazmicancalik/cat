...
id = get_run_id()
wandb.init(project=project, name="PT_PARTSEG_w_CONV_"+id, id=id, dir='wandb/runs', **kwargs)
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            wandb.log({"loss": loss.item()}) # Wandb loss log

            loss.backward()
            optimizer.step()

        train_instance_acc = np.mean(mean_correct)
        wandb.log({"train_avg_instance_acc":train_instance_acc})
        ...
        with torch.no_grad():
            ...
            for cat in sorted(shape_ious.keys()):
                ...
                wandb.log({f'test_{cat}_mIoU': shape_ious[cat]})
            test_metrics['class_avg_iou'] = mean_shape_ious
            test_metrics['inctance_avg_iou'] = np.mean(all_shape_ious)

            
            wandb.log({"test_avg_instance_acc": test_metrics['accuracy'],"test_avg_class_acc": test_metrics['class_avg_accuracy']})
            wandb.log({"test_avg_instance_iou": test_metrics['inctance_avg_iou'],"test_avg_class_iou": test_metrics['class_avg_iou']})
        ...
        save_list = [18,20,19]
        if (test_metrics['inctance_avg_iou'] >= best_inctance_avg_iou or epoch in save_list):
            logger.info('Save model...')
            savepath = f'best_model_{epoch}.pth'
        ...
        wandb.log({'best_test_avg_instance_acc': best_acc})
        wandb.log({'best_test_avg_class_iou': best_class_avg_iou})
        wandb.log({'best_test_instance_avg_iou': best_inctance_avg_iou})

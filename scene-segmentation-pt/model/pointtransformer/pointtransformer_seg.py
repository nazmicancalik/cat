...
from model.pointtransformer.convolution import ConvolutionBlock 
...
class PointTransformerLayer(nn.Module):
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        #print('\n\npoint transformer layer x before weird operation: ', x.size(), o)
        x = ((x_v + p_r).view(n, nsample, s, c // s) * w.unsqueeze(2)).sum(1).view(n, c)
        #print('\n\npoint transformer layer x at the end: ', x.size())
        return x
...
class PointTransformerBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, share_planes=8, nsample=16):
        ...
        if self.conv_enabled:
            self.convolution_block = ConvolutionBlock(planes, 31, 2) # Kernel size 31 and expansion=2

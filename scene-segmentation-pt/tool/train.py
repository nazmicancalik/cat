...
id = get_run_id()
wandb.init(project=project, name="PT_S3DIS_SEG_w_CONV_"+id, id=id, dir='wandb/runs', **kwargs)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        epoch_log = epoch + 1
        if main_process():
            wandb.log({"loss": loss_train, "epoch": epoch_log})
            wandb.log({"train_avg_instance_acc": allAcc_train})
            wandb.log({"train_avg_class_iou": mIoU_train})
            wandb.log({"train_avg_class_acc": mAcc_train})
        ...
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            ...
            if main_process():
                ...
                wandb.log({"val_avg_instance_acc": allAcc_val})
                wandb.log({"val_avg_class_iou": mIoU_val})
                wandb.log({"val_avg_class_acc": mAcc_val})
                ...
                wandb.log({"best_avg_class_iou": best_iou})
